/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package mx.edu.ux.congresos.service;

import java.util.List;
import mx.edu.ux.congresos.dao.AdministradorDAO;
import mx.edu.ux.congresos.model.Administrador;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

@Service("administradorService")
@Transactional
public class AdminitradorServiceImpl implements AdministradorService {

    @Autowired
    private AdministradorDAO dao;

    @Override
    public Administrador findById(int id) {
        return dao.findById(id);
    }

    @Override
    public void saveAdministrador(Administrador employee) {
        // TODO Auto-generated method stub
        dao.saveAdministrador(employee);
    }


    @Override
    public List<Administrador> findAllAdministrador() {
        return dao.findAllAdministrador();
    }

}
